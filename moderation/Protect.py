...
##            inter.response.send_message("What are you doing?")
    ...
    async def checkup(self, inter):
        ...
        await inter.response.send_message(embed=await statbed.create_success_embed(title="Dashboard Created!"), ephemeral=True)
    ...
    async def on_button_click(self, inter):
        ...
        if custom_id.startswith("fixDupe"):
            ...
            for role in deleteRoles:
                try:
                    ...
                    continue
            await inter.edit_original_response(embed=await statbed.create_success_embed(title="Done!"))
        ...
        if custom_id.startswith("fixProb"):
            ...
            if quarantine_role:
                ...
                await inter.edit_original_response(embed=await statbed.create_success_embed(title="Quarantine role has been made", description="Re-running fix..."))
                view_permission = disnake.Permissions(read_messages=False)
                try:
                    for channel in guild.channels:
        ...
        if custom_id.startswith("checkSpammer"):
            ...
            if not quarantine_role:
                quarantine_role = await guild.create_role(name="quarantined")
                for channel in guild.channels:
            ...
            for spammer in spammers:
                try:
                    await spammer.add_roles(quarantine_role)
                    quarantined_count += 1
                    quarantined_users.append(spammer.name)
                except disnake.Forbidden:
                    failed_users.append(spammer.name)
                    continue
            
            embed = await statbed.create_success_embed(
                title="Spammer Check Results",
                description=f"Quarantined {quarantined_count} out of {len(spammers)} detected spammers."
            )
            embed.add_field(name="Total Spammers", value=str(len(spammers)), inline=True)
            embed.add_field(name="Successfully Quarantined", value=str(quarantined_count), inline=True)
            embed.add_field(name="Failed to Quarantine", value=str(len(spammers) - quarantined_count), inline=True)
            
            more_detail_button = disnake.ui.Button(style=disnake.ButtonStyle.primary, label="More Detail", custom_id="more_detail_spammers", emoji="<:Search:1262184347016892530>")
            
            # Store the results in a way that can be accessed later
            self.spammer_check_results = {
                "quarantined_users": quarantined_users,
                "failed_users": failed_users
            }
            
            await inter.edit_original_response(embed=embed, components=[more_detail_button])

        elif custom_id == "more_detail_spammers":
            await inter.response.defer(with_message=True, ephemeral=True)
            
            detail_embed = disnake.Embed(title="Detailed Spammer Check Results", color=disnake.Color.orange())
            
            if self.spammer_check_results["quarantined_users"]:
                detail_embed.add_field(name="Quarantined Users", value="\n".join(self.spammer_check_results["quarantined_users"][:20]) + ("..." if len(self.spammer_check_results["quarantined_users"]) > 20 else ""), inline=False)
            else:
                detail_embed.add_field(name="Quarantined Users", value="None", inline=False)
            
            if self.spammer_check_results["failed_users"]:
                detail_embed.add_field(name="Failed to Quarantine", value="\n".join(self.spammer_check_results["failed_users"][:20]) + ("..." if len(self.spammer_check_results["failed_users"]) > 20 else ""), inline=False)
            else:
                detail_embed.add_field(name="Failed to Quarantine", value="None", inline=False)
            
            await inter.edit_original_response(embed=detail_embed)
    ...
    async def on_select(self, select: disnake.ui.Select, inter: disnake.ApplicationCommandInteraction):
        ...
        if message_id:
            ...
            if guild:
                ...
                # Add this new check after the existing checks
                spammer_count = sum(1 for member in guild.members if member.public_flags.spammer)
                if spammer_count > 0:
                    features.append(f"<:SecurityBad:1126200375125483520>│{spammer_count} spammer(s) detected in the server!")
...
We checked your server for any potentially dangerous permissions, missing roles, or unquarantined channels! Here are your results:"""
                ...
                # Add this new button to the homeRow list
                if spammer_count > 0:
                    checkSpammerButton = Button(style=disnake.ButtonStyle.red, label=f"Quarrantine {spammer_count} Spammer(s)", emoji="<:PerAlert:1263991129179951164>", custom_id=f"checkSpammer_{guild.id}")
                    homeRow.append(checkSpammerButton)
