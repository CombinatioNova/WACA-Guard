    ...
    async def on_message(self, message):
        ...
        try:
            ...
            if int(message.content) == next_number:
                if message.author == self.last_authors[guild_id] and next_number != 1:
                    await message.add_reaction("<:WACAx:1079146014969180340>")
                    await message.channel.send(f"{message.author.mention}, you repeated the number! Let's start over.")
                    self.counters[guild_id] = 0
                else:
                ...
                await message.add_reaction("<:WACAx:1079146014969180340>")
                await message.channel.send(f"{message.author.mention}, that's the wrong number! Let's start over.")
                self.counters[guild_id] = 0
        except ValueError:
